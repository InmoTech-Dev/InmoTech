...
-- =====================================================================================================================
-- PASO 1: Eliminar la constraint única automática que sigue bloqueando por 'nombre'
-- (Hacemos la búsqueda dinámica para no depender del nombre exacto)
-- =====================================================================================================================
...
WHERE kc.parent_object_id = OBJECT_ID('Comodidades')
  AND kc.type = 'UQ'
  AND kc.name != 'UQ_Comodidades_Nombre_Tipo'
  AND c.name = 'nombre'
GROUP BY kc.name
HAVING COUNT(*) = 1; -- Solo tiene 1 columna (solo 'nombre')

IF @constraintName IS NOT NULL
BEGIN
    EXEC('ALTER TABLE Comodidades DROP CONSTRAINT [' + @constraintName + ']');
    PRINT '✅ Constraint eliminada: ' + @constraintName;
END
ELSE
BEGIN
    PRINT '⚠️ No se encontró constraint adicional. Puede que ya esté limpia.';
END
GO

-- =====================================================================================================================
-- PASO 2: Limpiar los datos insertados por error (con nombres combinados)
-- =====================================================================================================================
DELETE FROM Comodidades 
WHERE nombre IN (
    'Habitaciones Casa', 'Baños Casa', 'Parqueaderos Casa',
    'Habitaciones Apartamento', 'Baños Apartamento',
    'Habitaciones Finca'
);

PRINT '✅ Datos incorrectos eliminados: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' filas borradas';
GO

-- =====================================================================================================================
-- PASO 3: Insertar las comodidades correctas
-- =====================================================================================================================
IF NOT EXISTS (SELECT 1 FROM Comodidades WHERE nombre = 'Habitaciones' AND tipo_inmueble = 'Casa')
BEGIN
    ...
    PRINT '✅ Comodidades insertadas correctamente (44 registros)';
END
ELSE
BEGIN
    PRINT '⚠️ Las comodidades correctas ya existen';
END
GO
