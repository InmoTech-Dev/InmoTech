  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        message:
          error.message || "No pudimos procesar la solicitud. Intenta nuevamente.",
      });
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="rounded-full p-1 text-slate-500 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed"
            aria-label="Cerrar"
            disabled={submitting}
          >
            <X className="h-4 w-4" />
        ...
        <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2 pt-2">
            <button
              type="button"
              onClick={handleRequestClose}
              className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={submitting}
            >
              Cancelar
            </button>
